...
// Consists of memoized TableRow rendered in a Table
...
import { DATE_FORMATS, Conversation } from '@/models';
import { dayjs } from '@/lib/utils';

const ConversationTableRow = memo((props: { convo: Conversation }) => {
    const { convo } = props;
    ...
    const { convo: oldConvo } = prevProps;
    const { convo: newConvo } = nextProps;
    return oldConvo.finished === newConvo.finished && oldConvo.review?.feedback === newConvo.review?.feedback && oldConvo.review?.rating === newConvo.review?.rating;
});
            ...
            <Table>
                ...
                <TableBody>
                    {paginatedConvos?.data.map((convo) => (
                        <ConversationTableRow convo={convo} key={convo.id} />
                    ))}
                </TableBody>
